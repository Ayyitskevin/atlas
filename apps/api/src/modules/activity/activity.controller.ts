...
export class ActivityController {
  ...
  listWorkspaceActivity = async (request: FastifyRequest) => {
    ...
    return this.service.listActivity(await requireAuth(request), workspaceId, parseQuery(request, activityQuerySchema));
  };
  listProjectActivity = async (request: FastifyRequest) => {
    ...
    return this.service.listActivity(await requireAuth(request), workspaceId, { ...query, projectId });
  };
  listTaskActivity = async (request: FastifyRequest) => {
    const { taskId, workspaceId } = parseParams(request, taskParamsSchema);
    const query = parseQuery(request, activityQuerySchema);
    return this.service.listActivity(await requireAuth(request), workspaceId, { ...query, taskId });
  };
}
