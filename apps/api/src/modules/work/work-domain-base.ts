...
export class WorkDomainBase {
  ...
  protected get sectionsRepo() {
    return this.workRepository.sections;
  }
  protected get tasksRepo() {
    return this.workRepository.tasks;
  }
  protected get labelsRepo() {
    return this.workRepository.labels;
  }
  protected get dependenciesRepo() {
    return this.workRepository.dependencies;
  }
  protected get subtasksRepo() {
    return this.workRepository.subtasks;
  }
  protected get commentsRepo() {
    return this.workRepository.comments;
  }
  protected get attachmentsRepo() {
    return this.workRepository.attachments;
  }
  protected get activityRepo() {
    return this.workRepository.activity;
  }
  protected get notificationsRepo() {
    return this.workRepository.notifications;
  }
  protected get searchRepo() {
    return this.workRepository.searchRepo;
  }
  ...
  protected async getTask(ctx: AuthContext, workspaceId: string, taskId: string) {
    ...
    const task = await this.tasksRepo.findTask(workspaceId, taskId);
  ...
  protected async scanInitialAttachment(input: {
    ...
    if (blockReason) {
      await this.attachmentsRepo.recordAttachmentScanResult({ attachmentId: input.attachmentId, scan, workspaceId: input.workspaceId });
  ...
  protected async scanAttachmentVersion(input: {
    ...
    const blockReason = attachmentScanBlockReason(scan);
    if (blockReason) {
      await this.attachmentsRepo.recordAttachmentVersionScanResult({ scan, versionId: input.versionId, workspaceId: input.workspaceId });
      this.throwAttachmentScanConflict(input.objectKey, scan, blockReason);
    }
    return scan;
  }
  ...
  protected async requireSectionsInProject(workspaceId: string, projectId: string, sectionIds: string[]) {
    ...
    const count = await this.sectionsRepo.countSections({ projectId, sectionIds: ids, workspaceId });
  ...
  protected async requireWorkspaceMembers(workspaceId: string, userIds: string[]) {
    ...
    const count = await this.tasksRepo.countWorkspaceMembers({ userIds: ids, workspaceId });
    if (count !== ids.length) {
  ...
  protected async dependencySummaryMap(workspaceId: string, taskIds: string[]) {
    ...
    const rows = await this.dependenciesRepo.listTaskDependencySummaryRows({
      taskIds: ids,
      workspaceId,
    });
  ...
  protected async requireTaskNotBlocked(workspaceId: string, taskId: string) {
    const rows = await this.dependenciesRepo.listTaskDependencySummaryRows({ taskIds: [taskId], workspaceId });
  ...
  protected async recordTasksUnblockedByCompletion(
    ...
    const rows = await this.dependenciesRepo.listTasksUnblockedByCompletion({
  ...
  protected async createNextRecurringTask(
    ...
    try {
      const nextTask = await this.tasksRepo.createRecurringTask({
